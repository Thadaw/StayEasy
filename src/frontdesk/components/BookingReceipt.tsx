...
export function BookingReceipt({
  ...
  const { data: property } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const { data } = await api.get(`/properties/${currentPropertyId}`, { skipAuthRedirect: true } as AuthRequestConfig)
  ...
  const { data: booking, isLoading } = useQuery({
    queryKey: ["booking-receipt", currentPropertyId, bookingId],
    queryFn: async (): Promise<Booking | null> => {
      if (!currentPropertyId || !bookingId) return null
      try {
    ...
    enabled: !!currentPropertyId && !!bookingId,
  })

  useEffect(() => {
    if (autoPrint && booking) window.print()
  }, [autoPrint, booking])

  if (isLoading) {
    return (
      <div className={embedded ? "flex items-center justify-center py-16" : "min-h-screen bg-gray-50 flex items-center justify-center"}>
  ...
  if (!booking) {
    return (
      <div className={embedded ? "py-16 text-center bg-gray-50" : "min-h-screen bg-gray-50 flex items-center justify-center"}>
