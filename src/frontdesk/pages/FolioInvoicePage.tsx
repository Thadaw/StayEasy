...
export default function FolioInvoicePage() {
  ...
  const { data: guestFolio } = useQuery({
    queryKey: ["folio-guest-info", currentPropertyId, folio?.booking_id],
    queryFn: async () => {
  ...
  const otherCharges = folio.charges.filter((c) => c.category?.toUpperCase() !== "ROOM_CHARGE")
  const roomNames = guestFolio?.rooms?.map((r) => r.room_name).join(", ") || "—"
          ...
          <div className="grid grid-cols-2 gap-8 mb-8 items-start">
            <div className="space-y-3">
              <div className="flex justify-between border-b border-gray-100 pb-2">
                ...
                <span className="text-sm font-semibold text-gray-900">{folio.guest_name || guestFolio?.guest_name || "—"}</span>
              </div>
              <div className="flex justify-between border-b border-gray-100 pb-2">
                <span className="text-sm text-gray-500">Email</span>
                <span className="text-sm font-semibold text-gray-900">{folio.guest_email || guestFolio?.guest_email || "—"}</span>
              </div>
              <div className="flex justify-between border-b border-gray-100 pb-2">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between bg-green-50 px-3 py-2 rounded-lg mt-1">
                ...
                <span className="font-bold text-green-700">{folio.settled_at ? "Paid" : "Unpaid"}</span>
              </div>
            </div>
          </div>
